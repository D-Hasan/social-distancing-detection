...
if __name__ == '__main__':
    ...
    VIDEO_PATH = 'datasets/TownCentreXVID.avi'
    cap = cv2.VideoCapture(VIDEO_PATH)
    total_frames = cap.get(7)

    # Specify number of frames to analyze
    num_frames = 400
    assert num_frames <= total_frames

    # Specify transform matrix
    MATRIX_PATH = 'calibration/oxford_town_matrix_cam2world.txt'
    transform_cam2world = np.loadtxt(MATRIX_PATH)

    # Specify path of output pickle file
    pickle_path = './oxford_400_frames.p'
    ...
    # Run
    main(cap=cap, detector=detector, transform_cam2world=transform_cam2world,
         num_frames=num_frames, pickle_path=pickle_path, vis=vis, vis_dir=VIS_DIR)
